...
// Payment method display labels & colors
const ML = {
  CASH:                 'Cash',
  CARD:                 'Card',
  CREDIT_CARD:          'Card',
  FULLY_CREDIT:         'Full Credit',
  PARTIAL:              'Partial',
  FULL_PAYMENT:         'Full Payment',
  OUTSTANDING_SETTLEMENT: 'Settlement',
}
const MC = {
  CASH:                 '#2e7d32',
  CARD:                 '#1565c0',
  ...
  PARTIAL:              '#e65100',
  FULL_PAYMENT:         '#2e7d32',
  OUTSTANDING_SETTLEMENT: '#6a1b9a',
}

const TT = {
  contentStyle: {
    background: '#fff', border: '1px solid #e0e0e0', borderRadius: 8,
    color: '#333', fontSize: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.1)',
  },
}
...
function KpiCard({ label, value, sub, accent, icon, chip }) {
  return (
    <Card elevation={0} sx={{
      ...
      borderLeft: `4px solid ${accent}`, borderRadius: 2, height: '100%',
      transition: 'box-shadow .2s', '&:hover': { boxShadow: '0 4px 20px rgba(0,0,0,0.08)' },
    }}>
      <CardContent sx={{ p: 2.5 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography variant="caption" sx={{ color: 'text.secondary', textTransform: 'uppercase', letterSpacing: .8, fontWeight: 600, display: 'block' }}>
              {label}
            </Typography>
            <Typography variant="h5" sx={{ fontWeight: 800, mt: .5, color: 'text.primary', lineHeight: 1.2 }}>
              {value}
            </Typography>
...
function StatBadge({ status, isSettlement }) {
  ...
  const m = {
    COMPLETED: { label: 'Paid',    bg: '#e8f5e9', color: '#2e7d32' },
...
// Custom Y-axis formatter: auto K/M
const yFmt = (v) => {
...
export default function SalesReportPage() {
  ...
  // ── Net profit = Revenue (invoiced) - Expenses (from vouchers) ─────────────
  // If no expense vouchers exist yet, show gross profit note
  const netProfit     = totalRevenue - totalExpenses
  const profitMargin  = totalRevenue > 0 ? (netProfit / totalRevenue) * 100 : 0
  // Collection rate = cash collected / total invoiced
  ...
  // For bar chart: use date label shortened, ensure numbers
  const cfChart = salesByDate.map(d => ({
  ...
  const handleExportCSV = () => {
    const csvRows = [
      ...
      ['Total Outstanding', totalOutstanding], ['Credit Recovered (Settlements)', outstandingSettled],
      ['Net Profit', netProfit], ['Profit Margin', `${profitMargin.toFixed(1)}%`],
      ['Collection Rate', `${collectionRate.toFixed(1)}%`],
      ['Total Transactions', totalTransactions], ['Avg Ticket', avgTicket.toFixed(0)],
      ['Refunds', refundTotal], [],
      ['=== PAYMENT BREAKDOWN ==='],
      ['Cash Sales', cashSalesAmt, `${fullyPaidCount} txns`],
      ['Card Sales', cardSalesAmt, ''],
      ['Full Credit', fullyCreditTotal, `${fullyCreditCount} txns`],
      ['Partial Collected', partialCollected, `${partialCount} txns`],
      ['Partial Outstanding', partialCredit, ''],
      ['Discounts', discounts], [],
  ...
  const handleExportPDF = () => {
    ...
    <div class="g">
      ...
      <div class="c"><div class="cv" style="color:#2e7d32">PKR ${fmt(totalCashReceived)}</div><div class="cl">Cash Collected</div></div>
        ...
        <Box sx={{ p: { xs: 2, md: 3 }, bgcolor: 'grey.50', minHeight: '100vh' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3, flexWrap: 'wrap', gap: 2 }}>
            <Box>
              ...
              <Typography variant="h4" sx={{ fontWeight: 800, color: 'text.primary', mt: .5, letterSpacing: '-.5px' }}>
                Sales Report
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: .5 }}>
                Revenue · Cash collected · Credit · Partial · Profit
              </Typography>
            </Box>
          ...
          <Grid container spacing={2} sx={{ mb: 2 }}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <KpiCard
                label={netProfit >= 0 ? 'Net Profit (Gross)' : 'Net Loss'}
                value={fmtPKR(Math.abs(netProfit))}
                sub={totalExpenses > 0
                  ? `${profitMargin.toFixed(1)}% margin · Expenses PKR ${fmt(totalExpenses)}`
                  : `Gross — no expense vouchers entered`}
          ...
          {/* ── KPI Row 2 — Payment method cards ─────────────────────────── */}
          <Grid container spacing={2} sx={{ mb: 2 }}>
            <Grid item xs={6} sm={3}>
            ...
            <Grid item xs={6} sm={3}>
              <KpiCard label="Full Credit" value={fmtPKR(fullyCreditTotal)} sub={`${fullyCreditCount} invoices · 0 collected`} accent="#c62828" icon={<AccountBalance />} />
            </Grid>
            <Grid item xs={6} sm={3}>
              <KpiCard label="Partial Sales" value={fmtPKR(partialTotal)}
                sub={partialCount > 0 ? `Collected ${fmtPKR(partialCollected)} · Due ${fmtPKR(partialCredit)}` : `${partialCount} partial sales`}
                accent="#e65100" icon={<Receipt />} />
            </Grid>
          </Grid>

          {/* ── KPI Row 3 — Recovery & refunds ───────────────────────────── */}
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={6} sm={3}>
              <KpiCard label="Credit Recovered" value={fmtPKR(outstandingSettled)}
                sub={`${outstandingSettlementCount} settlement${outstandingSettlementCount !== 1 ? 's' : ''}`}
                accent="#6a1b9a" icon={<SwapHoriz />} />
            </Grid>
            <Grid item xs={6} sm={3}>
              <KpiCard label="Refunds Issued" value={fmtPKR(refundTotal)}
                sub={`${refundCount} refund${refundCount !== 1 ? 's' : ''}`}
                accent="#b71c1c" icon={<TrendingDown />} />
            </Grid>
            <Grid item xs={6} sm={3}>
              <KpiCard label="Discounts Given" value={fmtPKR(discounts)} sub="Total discount on sales" accent="#f57c00" icon={<Receipt />} />
            </Grid>
            <Grid item xs={6} sm={3}>
              <KpiCard label="Avg Ticket" value={fmtPKR(avgTicket)} sub={`Per real sale`} accent="#0288d1" icon={<ShoppingCart />} />
            </Grid>
          </Grid>

          {/* ── Charts Row 1 ──────────────────────────────────────────────── */}
          <Grid container spacing={2.5} sx={{ mb: 3 }}>
            {/* Daily Collected vs Credit */}
              ...
              <Paper elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, p: 2.5, height: '100%' }}>
                ...
                {expBreakdown.length === 0 ? (
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 230, gap: 1 }}>
                    ...
                    <Typography color="text.disabled" variant="caption" sx={{ textAlign: 'center', maxWidth: 240 }}>
                      Add expense vouchers to see profit margin accurately
                    </Typography>
                  </Box>
                ) : (
          ...
          <Paper elevation={0} sx={{
            border: '2px solid', borderRadius: 2, p: 3, mb: 3,
            borderColor: netProfit >= 0 ? 'success.light' : 'error.light',
            bgcolor:     netProfit >= 0 ? '#f1f8f2'       : '#fdf2f2',
          }}>
              ...
              <Grid item xs={12} md={4}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="h3" sx={{ fontWeight: 800, color: netProfit >= 0 ? 'success.dark' : 'error.dark', lineHeight: 1 }}>
                      PKR {fmt(Math.abs(netProfit))}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ mt: .5 }}>
                      {profitMargin.toFixed(1)}% margin · {collectionRate.toFixed(1)}% collected
                    </Typography>
                    {totalExpenses === 0 && (
                      <Typography variant="caption" sx={{ color: 'warning.main', display: 'block', mt: .5 }}>
                        ⚠ No expenses recorded — showing gross profit
                      </Typography>
                    )}
                  </Box>
                </Box>
              ...
              <Grid item xs={12} md={8}>
                <Grid container spacing={2}>
                  {[
                    ...
                    { label: 'Credit Recovered',     value: fmtPKR(outstandingSettled),    color: '#6a1b9a'        },
                    { label: 'Refunds',              value: fmtPKR(refundTotal),           color: 'text.secondary' },
                  ].map(item => (
                      ...
                      <TableRow>
                        {['Cashier', 'Invoiced', 'Collected', 'Outstanding', 'Txns'].map(h => (
                          <TableCell key={h} align={h === 'Cashier' ? 'left' : 'right'}
                            sx={{ bgcolor: 'primary.50', color: 'primary.dark', fontWeight: 700, fontSize: '.68rem', textTransform: 'uppercase', letterSpacing: .5 }}>
                            {h}
                          </TableCell>
                        ))}
                      </TableRow>
                ...
                <TableHead>
                  <TableRow>
                    {['Date', 'Invoice', 'Customer', 'Cashier', 'Total', 'Collected', 'Outstanding', 'Method', 'Status'].map(h => (
                      <TableCell key={h} sx={{ bgcolor: 'primary.50', color: 'primary.dark', fontWeight: 700, fontSize: '.68rem', textTransform: 'uppercase', letterSpacing: .5, whiteSpace: 'nowrap' }}>
                        {h}
                      </TableCell>
                    ))}
                  </TableRow>
                ...
                <TableBody>
                  {recentSales.length === 0 ? (
                    ...
                    <TableRow key={i} sx={{
                      ...
                      bgcolor: row.is_settlement ? '#f8f0ff'
                        : row.credit_amount > 0 && row.payment_amount === 0 ? '#fff8f8'
                        : 'inherit',
                    }}>
                      <TableCell sx={{ fontSize: '.75rem', color: 'text.secondary', whiteSpace: 'nowrap' }}>{row.date}</TableCell>
                      <TableCell sx={{ fontSize: '.75rem', fontWeight: 600, whiteSpace: 'nowrap', color: row.is_settlement ? '#6a1b9a' : 'primary.main' }}>
                        {row.invoice_no}
                      </TableCell>
                      ...
                      <TableCell sx={{ fontSize: '.8rem', fontWeight: 700, whiteSpace: 'nowrap' }}>
                        {row.is_settlement ? '—' : `PKR ${fmt(row.total)}`}
                      </TableCell>
                      <TableCell sx={{ fontSize: '.8rem', fontWeight: 600, color: 'success.dark', whiteSpace: 'nowrap' }}>
                        PKR {fmt(row.payment_amount)}
                      </TableCell>
                    ...
                    <TableRow>
                      {['Branch', 'Invoiced', 'Collected', 'Outstanding', 'Txns', 'Collection %'].map(h => (
                        <TableCell key={h} align={h === 'Branch' ? 'left' : 'right'}
                          sx={{ bgcolor: 'primary.50', color: 'primary.dark', fontWeight: 700, fontSize: '.7rem', textTransform: 'uppercase' }}>
                          {h}
                        </TableCell>
                      ))}
                    </TableRow>
