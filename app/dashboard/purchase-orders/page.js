...
function PurchaseOrdersPage() {
  ...
  // ── formData — initialized with effective user scope ──────────────────────
  const getInitialFormData = useCallback(() => ({
    supplierId:       '',
    scopeType:        user?.role === 'CASHIER'          ? 'BRANCH'
                    : user?.role === 'WAREHOUSE_KEEPER'  ? 'WAREHOUSE'
                    : '',
    scopeId:          user?.role === 'CASHIER'          ? user?.branchId
  ...
  // Re-initialise formData when user scope becomes available (after URL params parsed)
  useEffect(() => {
    if (!initialized) return
    setFormData(getInitialFormData())
  }, [initialized, getInitialFormData])

  // ── Fetch POs — wait for URL params to be read ────────────────────────────
  useEffect(() => {
    if (!initialized) return
  ...
  // ── Load inventory for item search dropdown ───────────────────────────────
  useEffect(() => {
    const loadInventory = async () => {
      try {
        ...
        const res  = await api.get('/inventory', {
          params: { scopeType: formData.scopeType, scopeId: formData.scopeId, limit: 'all' }
        })
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const data = res.data?.data || res.data || []
        setCategoriesFromApi(Array.isArray(data)
  ...
  const handleItemSearchChange = (i, v) => {
  ...
  // Scope Type field is read-only for CASHIER / WAREHOUSE_KEEPER (even in simulation)
        ...
        <Grid item xs={12} md={4}>
          <FormControl fullWidth error={!!formErrors.supplierId} sx={{ minWidth:180 }}>
            ...
            <Select value={formData.supplierId} label="Supplier *"
              onChange={(e) => handleFieldChange('supplierId', e.target.value)} size="small"
    ...
    <DialogActions sx={{ p:3, borderTop:1, borderColor:'divider', justifyContent:'space-between' }}>
      <Box sx={{ display:'flex', gap:1 }}>
        ...
        <Button variant="outlined" color="error" startIcon={<DeleteSweepIcon />} size="large"
          disabled={selectedRows.length === 0} onClick={deleteSelectedRows}>
          Delete Selected {selectedRows.length > 0 ? `(${selectedRows.length})` : ''}
        </Button>
      </Box>
      <Box sx={{ display:'flex', gap:1 }}>
    ...
    <RouteGuard allowedRoles={['ADMIN', 'WAREHOUSE_KEEPER', 'CASHIER']}>
      <DashboardLayout>
                ...
                <FormControl fullWidth size="small">
                  ...
                  <Select value={filters.supplierId} label="Supplier"
                    ...
                    renderValue={(v) => {
                      ...
                      const s = suppliers.find(s => s.id===v || s.id===Number(v))
                    ...
                    {suppliers.map(s => (
                      <MenuItem key={s.id} value={s.id}>
                        <Typography noWrap sx={{ maxWidth:300 }}>{s.name}</Typography>
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
                ...
                <Table>
                  ...
                  <TableBody>
                    {purchaseOrders.map((order, index) => {
                      ...
                      const orderStatus = (raw && String(raw) !== 'null' && String(raw) !== 'undefined')
                        ? String(raw).trim().toUpperCase()
                        : 'PENDING'
                      const cfg = statusConfig[orderStatus]

                      // Show edit button for admin OR for scoped user who owns this order
                      const canEdit =
                        user?.role === 'ADMIN' ||
                        (orderStatus === 'PENDING' && (
                      ...
                      return (
                        <TableRow key={order.id} hover>
                          ...
                          <TableCell>
                            <Typography variant="body2">{new Date(order.orderDate).toLocaleDateString()}</Typography>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <Chip icon={cfg?.icon} label={cfg?.label||orderStatus} color={cfg?.color||'default'} size="small" />
                          </TableCell>
                          ...
                          <TableCell>
                            <Box sx={{ display:'flex', gap:0.5 }}>
                              ...
                              {/* Status actions — admin only (or simulated admin) */}
            ...
            {pagination.totalPages > 1 && (
              <Box sx={{ display:'flex', justifyContent:'center', mt:2 }}>
                <Pagination count={pagination.totalPages} page={pagination.page}
                  onChange={(e,p) => dispatch(setPagination({ page:p }))} color="primary" />
              </Box>
            )}
          </CardContent>
        </Card>
        ...
        <Dialog open={formDialogOpen} onClose={() => setFormDialogOpen(false)}
          maxWidth={false} fullWidth PaperProps={dialogPaperProps}>
          ...
          <ActionButtons onCancel={() => setFormDialogOpen(false)} onSubmit={handleSubmit}
            submitLabel="Create Order" loadingLabel="Creating..." />
        </Dialog>

        {/* ── Edit Dialog ───────────────────────────────────────────────── */}
        <Dialog open={editDialogOpen} onClose={() => { setEditDialogOpen(false); resetForm() }}
          maxWidth={false} fullWidth PaperProps={dialogPaperProps}>
          <FormHeader title={`Edit Purchase Order #${editingOrder?.orderNumber}`}
          ...
          <ActionButtons onCancel={() => { setEditDialogOpen(false); resetForm() }}
            onSubmit={handleUpdateSubmit} submitLabel="Update Order" loadingLabel="Updating..." />
        </Dialog>
                      ...
                      <Stack spacing={1}>
                        ...
                        <Box sx={{ display:'flex', gap:1 }}>
                          ...
                          {(() => {
                            ...
                            const k   = (raw && String(raw) !== 'null') ? String(raw).trim().toUpperCase() : 'PENDING'
                            const c   = statusConfig[k]
                            return <Chip icon={c?.icon} label={c?.label||k} color={c?.color||'default'} size="small" />
                          })()}
                        </Box>
                        <Box sx={{ display:'flex', gap:1 }}>
                    ...
                    <TableBody>
                      ...
                      <TableRow>
                        <TableCell colSpan={7} align="right">
                          <Typography variant="body2" fontWeight="bold">Grand Total Cost:</Typography>
                        </TableCell>
                        <TableCell align="right">
                          <Typography variant="body2" fontWeight="bold" color="primary.main">
                            {parseFloat(selectedOrder.totalAmount||0).toFixed(2)}
