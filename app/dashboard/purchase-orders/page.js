...
function PurchaseOrdersPage() {
    ...
    const loadInventory = async () => {
      try {
        ...
        setInventoryOptions(Array.isArray(data)
          ? data.map(i => ({ label: i.name, name: i.name, sku: i.sku, category: i.category || 'General', costPrice: i.cost_price, sellingPrice: i.selling_price })).filter(o => o.name)
          : [])
  ...
  }, [handleItemChange, updateItemSearchState])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      resetForm()
      dispatch(fetchPurchaseOrders(filters))
    } catch (e) { console.error('Error creating purchase order:', e) }
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      ...
      setEditingOrder(null)
      dispatch(fetchPurchaseOrders(filters))
    } catch (e) { console.error('Error updating purchase order:', e) }
    finally { setIsSubmitting(false) }
  }
