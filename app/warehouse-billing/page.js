...
function WarehouseBillingPage() {
      ...
      if (createWarehouseSale.fulfilled.match(result)) {
        ...
        const pd = {
          ...
          companyPhone: companyInfo.phone, companyEmail: companyInfo.email, logoUrl: companyInfo.logoUrl,
              receiptNumber: sale.invoice_no || sale.invoiceNumber || `INV-${Date.now()}`,
            date: formatDisplayDate(sale.sale_date || sale.saleDate || sale.created_at || sale.createdAt || Date.now()),
            time: new Date(sale.sale_date || sale.saleDate || sale.created_at || sale.createdAt || Date.now()).toLocaleTimeString(),
          cashierName: user?.name || user?.username || 'Warehouse Keeper',
          customerName: selectedRetailer?.name || customerName || 'Walk-in',
          customerPhone: selectedRetailer?.phone || customerPhone || '',
          items: printItems,
          subtotal: Math.round(parseFloat(sale.subtotal ?? sale.totalAmount ?? 0)),
          tax: Math.round(parseFloat(sale.tax ?? sale.taxAmount ?? 0)),
          discount: Math.round(parseFloat(sale.discount ?? sale.discountAmount ?? 0)),
          total: Math.round(parseFloat(sale.total ?? sale.finalAmount ?? 0)),
  ...
  const printToBrowser = async (printData) => {
    try {
      ...
      printWindow.document.write(`<html><head><title>Receipt</title><style>@media print{body{margin:0;}@page{margin:0;size:80mm auto;}}</style></head><body><div style="font-family:'Courier New',Courier,monospace;width:80mm;max-width:80mm;margin:0;padding:2px 1px;box-sizing:border-box;font-size:12px;line-height:1.28;color:#000;"><div style="text-align:center;margin-bottom:6px;"><img src="${resolvedLogoPath}" style="max-width:100%;width:${logoSizes.cssWidth};height:auto;filter:grayscale(100%);display:block;margin:0 auto 4px;"><div style="font-size:14px;font-weight:900;letter-spacing:0.2px;word-break:break-word;">${printData.companyName || ''}</div><div style="font-size:10px;">${printData.companyAddress || ''}</div><div style="font-size:10px;">Tel: ${printData.companyPhone || ''}</div></div><div style="border-top:1px dashed #000;margin:4px 0 2px;"></div><div style="font-weight:900;text-align:center;font-size:12px;text-transform:uppercase;letter-spacing:0.8px;margin:2px 0;">${printData.title || 'SALES RECEIPT'}</div><div style="border-top:1px dashed #000;margin:2px 0 6px;"></div><div style="display:flex;justify-content:space-between;margin-bottom:2px;font-size:11px;font-weight:700;"><span>Receipt #</span><span style="font-weight:900;">${printData.receiptNumber || ''}</span></div><div style="display:flex;justify-content:space-between;margin-bottom:2px;font-size:11px;font-weight:700;"><span>Date/Time</span><span>${printData.date} ${printData.time || ''}</span></div><div style="display:flex;justify-content:space-between;margin-bottom:2px;font-size:11px;font-weight:700;"><span>Cashier</span><span>${printData.cashierName || ''}</span></div><div style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:11px;font-weight:700;"><span>Retailer</span><span>${printData.customerName || ''}</span></div><div style="border-top:2px solid #000;margin:4px 0;"></div><div style="display:flex;font-weight:900;font-size:11px;border-bottom:1px dashed #000;padding-bottom:3px;margin-bottom:4px;"><div style="flex:1;">ITEM</div><div style="width:32px;text-align:right;">QTY</div><div style="width:52px;text-align:right;">PRICE</div><div style="width:52px;text-align:right;">TOTAL</div></div>${(printData.items || []).map(item => `<div style="margin-bottom:5px;padding-bottom:3px;border-bottom:1px dotted #ccc;"><div style="font-weight:800;font-size:11px;">${item.name}</div><div style="display:flex;font-size:11px;"><div style="flex:1;"></div><div style="width:32px;text-align:right;font-weight:700;">${item.quantity}</div><div style="width:52px;text-align:right;font-weight:700;">${Math.round(item.unitPrice || 0)}</div><div style="width:52px;text-align:right;font-weight:900;">${Math.round(item.total || 0)}</div></div></div>`).join('')}<div style="border-top:2px solid #000;margin:4px 0;"></div><div style="display:flex;justify-content:space-between;margin-bottom:2px;font-size:11px;font-weight:700;"><span>Subtotal</span><span style="font-weight:800;">${Math.round(printData.subtotal || 0)}</span></div><div style="border:2px solid #000;margin:6px 0;padding:6px 4px;text-align:center;"><div style="font-weight:900;font-size:16px;">TOTAL  ${Math.round(printData.total || 0)}</div></div><div style="border-top:1px dashed #000;padding-top:4px;margin-bottom:4px;"><div style="font-weight:900;font-size:11px;margin-bottom:3px;">PAYMENT</div><div style="display:flex;justify-content:space-between;margin-bottom:2px;font-size:11px;font-weight:700;"><span>Method</span><span style="font-weight:800;">${printData.paymentMethod || 'CASH'}</span></div><div style="display:flex;justify-content:space-between;font-size:11px;font-weight:700;"><span>Paid</span><span style="font-weight:800;">${Math.round(printData.paymentAmount || 0)}</span></div></div><div style="border-top:2px solid #000;margin:6px 0 4px;"></div><div style="text-align:center;"><div style="font-size:11px;font-weight:800;margin-bottom:2px;">${printData.footerMessage || 'Thank you for choosing us!'}</div><div style="font-size:10px;margin-bottom:3px;">Return within 3 days with receipt</div><div style="border-top:1px dashed #000;margin:4px 0;"></div><div style="font-size:11px;font-weight:700;">Powered by Tychora</div><div style="font-size:10px;">www.tychora.com</div></div></div></body></html>`)
